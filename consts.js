
const {
    GOOGLE_SHEETS_PRIVATE_KEY = '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n',
    SPREADSHEET_ID = '1W4vN_3sWS-LI2JWrcUrkpuITcHYusCdF2fWKk1GDAtY',
    GOOGLE_SHEETS_CLIENT_EMAIL = 'high-demand-parts@high-demand-parts.iam.gserviceaccount.com',
    FUTURE_ELECTRONICS_KEY = 'XTUU8-M6GWC-4ZIQ9-AH9IW-OKXH5',
    FUTURE_ELECTRONICS_URL = 'https://api.futureelectronics.com/api/v1/pim-future'
} = process.env

module.exports = {
    GOOGLE_SHEETS_PRIVATE_KEY,
    SPREADSHEET_ID,
    GOOGLE_SHEETS_CLIENT_EMAIL,
    FUTURE_ELECTRONICS_KEY,
    FUTURE_ELECTRONICS_URL,
}